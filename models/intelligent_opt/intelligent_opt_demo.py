"""
用途: 智能优化算法综合演示 (GA, PSO, DE, SA)
输入: 无（内置测试函数）
输出: 控制台结果 + output/intelligent_opt_result.png
...
def sphere(x):
    """Sphere 单峰凸函数，全局最小 f(0)=0，范围 [-5.12, 5.12]"""
    return np.sum(x ** 2)


def rastrigin(x):
    """Rastrigin 多峰函数，大量局部极小，全局最小 f(0)=0，范围 [-5.12, 5.12]"""
    n = len(x)
    return 10 * n + np.sum(x ** 2 - 10 * np.cos(2 * np.pi * x))


def ackley(x):
    """Ackley 多峰函数，外平坦中心陡峭，全局最小 f(0)≈0，范围 [-32, 32]"""
...
def pso_minimize(func, n_vars, lb, ub, n_particles=50, max_iter=200,
    ...
    """粒子群优化主函数 (最小化)，带线性递减惯性权重，返回全局最优解、最优值、历史"""
    dim = n_vars
    span = ub - lb
...
def main():
    ...
    output_path = os.path.join(OUTPUT_DIR, 'intelligent_opt_result.png')
