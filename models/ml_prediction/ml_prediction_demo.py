"""
用途: 机器学习回归预测 — 四模型对比 (Linear, Ridge, RF, MLP)
输入: 无（自动生成合成数据）
输出: 控制台结果 + output/ml_prediction_result.png
调用: python ml_prediction_demo.py
"""
...
# 真实关系: y = 3*x1 + 2*x2 + 0.5*x3 + noise，x4/x5 为冗余噪声特征
...
for name, model in models.items():
    # Ridge/MLP 需标准化，Linear/RF 用原始数据
...
# 保存
output_path = os.path.join(OUTPUT_DIR, 'ml_prediction_result.png')
