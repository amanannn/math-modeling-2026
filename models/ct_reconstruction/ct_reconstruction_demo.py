"""
用途: CT 图像重建演示 — 从投影到 FBP 滤波反投影的完整流程
      展示：原始体模 → 正弦图（Radon 正变换）→ 直接反投影 → FBP 重建

输入: 无（自动生成多圆叠加体模，Radon 正变换使用解析公式）
输出: 四面板对比图 → output/ct_reconstruction_result.png
      控制台输出关键数值中间结果

调用示例:
    python ct_reconstruction_demo.py

依赖: numpy, scipy.fft, matplotlib (Agg 后端)
       （完全自包含，不依赖 scikit-image 等外部库）

算法说明:
    1. 使用多个椭圆叠加构建体模（Shepp-Logan 风格）
    2. Radon 正变换采用解析公式: 每个椭圆的投影有闭合形式
       → p(s,θ) = Σ 2*A_i*a_i*b_i / L_i² * sqrt(L_i² - (s - s0_i)²)
    3. 直接反投影: 将投影值沿射线方向"涂抹"回图像空间
    4. FBP: 频域乘以 Ram-Lak 斜坡滤波器 |ω| → 消除反投影的 1/|ρ| 模糊
"""
...
# 椭圆参数: (A, a, b, x0, y0, theta_deg)
#   A = 强度, a/b = 半轴, x0/y0 = 中心偏移, theta = 旋转角度(度)
SHEPP_LOGAN_PARAMS = [
...
def render_phantom(params, n_pixels=256):
    ...
    参数:
        params:   椭圆参数列表 [(A, a, b, x0, y0, theta_deg), ...]
        n_pixels: 图像像素数

    返回:
        phantom: (n_pixels, n_pixels) 的图像数组
    """
...
def radon_transform_analytic(params, angles_rad, n_detectors, half_range=1.4):
    ...
    解析计算多椭圆体模的 Radon 变换。
    每个椭圆的 Radon 变换有闭合形式:
      对旋转后的椭圆: p(s,θ) = 2*A*a*b / L² * sqrt(L² - (s - s0)²)

    参数:
        params:      椭圆参数列表 [(A, a, b, x0, y0, theta_deg), ...]
        angles_rad:  角度列表（弧度）
        n_detectors: 探测器单元数
        half_range:  探测器覆盖半宽（归一化坐标）

    返回:
        sinogram: (len(angles_rad), n_detectors) 的投影数据
    """
    ...
    for A, a, b, x0, y0, theta_deg in params:
        if A == 0:
            continue
        theta_e = np.deg2rad(theta_deg)  # 椭圆的旋转角
        cos_e, sin_e = np.cos(theta_e), np.sin(theta_e)

        for i, theta in enumerate(angles_rad):
            # 校正旋转后的有效半轴
            # 椭圆旋转后, 沿 θ 方向的投影半宽为:
            #   L² = a²*cos²(θ-θe) + b²*sin²(θ-θe)
            ...
            # 解析公式: p(s) = 2*A*a*b / L² * sqrt(L² - (s - s0)²)
            dist_sq = (s_pos - s0) ** 2
...
def back_project(sinogram, angles_rad, n_pixels, half_range=1.4):
    """
    直接反投影重建（不加滤波）。
    将每个角度的投影值沿对应方向"涂抹"回图像空间。

    参数:
        sinogram:    (n_angles, n_detectors) 投影数据
        angles_rad:  角度列表（弧度）
        n_pixels:    重建图像的像素数
        half_range:  探测器覆盖半宽（对应 phantom 中归一化坐标的覆盖范围）

    返回:
        recon: (n_pixels, n_pixels) 重建图像
    """
    n_angles, n_detectors = sinogram.shape
    center = n_pixels / 2.0
    scale = 2.0 * half_range / (n_detectors - 1)  # 每个探测器单元的宽度

    x = np.arange(n_pixels) - center
    X, Y = np.meshgrid(x / center, x / center)  # 归一化到 [-1, 1]
    ...
    # 反投影积分: ∫₀^π p_filt(s,θ) dθ ≈ (π/n_angles) * Σ p_filt
...
def fbp_reconstruct(sinogram, angles_rad, n_pixels, half_range=1.4):
    ...
    # ─── 步骤 A: 构建 Ram-Lak 斜坡滤波器 ───
    # 连续域: H(ω) = |ω|, 其中 ω 为角频率
    # 离散域: 使用 fftfreq 得到归一化频率 [-0.5, 0.5)
    #         实际频率 f = freqs / Δs, 角频率 ω = 2πf
    #         H[k] = |ω_k| = 2π|f_k| = 2π|k|/(N·Δs)
    # 实现中, 常数因子 2π/Δs 由反投影的积分缩放吸收
    ...
    # FBP 需要额外的 1/2 缩放因子（来自斜坡滤波器的离散化补偿）
    recon *= 0.5
    return recon
...
def normalize_for_comparison(recon, phantom, mask_thresh=0.05):
    ...
    这是因为 BP/FBP 的重建值尺度可能与原始体模不同。
    """
    mask = phantom > mask_thresh
    if mask.sum() < 10:
        mask = phantom > 0
    # 线性回归: recon_scaled = a * recon + b
...
def main():
    ...
    # (c) Single projection curve at angle 45
    ax = axes[0, 2]
    angle_idx = 45
