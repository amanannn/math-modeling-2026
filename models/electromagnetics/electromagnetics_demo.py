"""
用途: 计算电磁学与数学建模 Demo 脚本
      涵盖三个核心概念:
        (1) 均匀直线阵方向图 (阵列因子) — 极坐标 + 直角坐标
        (2) Friis 传输方程 — 自由空间路径损耗 vs 距离
        (3) 趋肤深度 vs 频率

输入: 无 (所有参数在脚本内定义)
输出: (1) 控制台打印数值结果
      (2) D:/虚拟C盘/数学建模培训/output/electromagnetics_result.png (300dpi)

调用: /c/Users/gu/miniconda3/python.exe electromagnetics_demo.py

依赖: numpy, matplotlib, scipy.constants
"""
...
def uniform_linear_array_factor(N, d, theta):
    """
    计算均匀直线阵的阵列因子 (Array Factor)
    参数:
        N     : int, 阵元数
        d     : float, 阵元间距 (归一化到波长 λ)
        theta : ndarray, 角度 (弧度), -π/2 ~ π/2
    返回:
        AF    : ndarray, 归一化阵列因子 (线性)
        AF_dB : ndarray, 归一化阵列因子 (dB)
    """
...
def path_loss_db(d, f):
    ...
    自由空间路径损耗 (Friis 传输方程)
    参数:
        d : float or ndarray, 距离 (m)
        f : float, 频率 (Hz)
    返回:
        L : float or ndarray, 路径损耗 (dB)
    """
    return 20 * np.log10(d) + 20 * np.log10(f) - 147.55


def demo_friis():
    """演示 Friis 路径损耗，返回 fig 对象"""
...
def skin_depth(f, sigma, mu_r=1.0):
    ...
    趋肤深度 (Skin Depth)
    参数:
        f     : float or ndarray, 频率 (Hz)
        sigma : float, 电导率 (S/m)
        mu_r  : float, 相对磁导率
    返回:
        delta : float or ndarray, 趋肤深度 (m)
    """
    return np.sqrt(1 / (pi * f * mu_0 * mu_r * sigma))


def demo_skin_effect():
    """演示趋肤深度 vs 频率，返回 fig 对象"""
    # 三种常见导体
    materials = {
