...
def solve_lp(c, A_ub, b_ub, bounds, maximize=True):
    """
    求解线性规划
    c: 目标系数; A_ub, b_ub: 不等式约束 A_ub @ x <= b_ub; bounds: 变量范围
    """
...
# Template 5: TOPSIS 综合评价
def topsis_template(X, weights, directions):
    """
    TOPSIS 综合评价
    X: m行n列 (m方案, n指标); weights: 权重; directions: 方向 (1=极大型, -1=极小型)
    """
    m, n = X.shape
    X_pos = X.copy()
    for j in range(n):
...
# Template 6: AHP 层次分析法
def ahp_template(matrix):
    """层次分析法权重计算，matrix 为 n×n 判断矩阵（Saaty 1~9 标度）"""
